...
export class UserController {
  ...
  async findManyUserN(
    ...
    const results = await this.service.findOne({ where: params }).userN({
      where: query,
      select: {
        createdAt: true,
        description: true,
  ...
  @common.UseInterceptors(nestMorgan.MorganInterceptor("combined"))
  @common.UseGuards(basicAuthGuard.BasicAuthGuard, nestAccessControl.ACGuard)
  @common.Get("/:id/userName")
  @nestAccessControl.UseRoles({
    resource: "User",
    action: "read",
    possession: "any",
  })
  async findManyUserName(
    @common.Param() params: UserWhereUniqueInput,
    ...
    const results = await this.service.findOne({ where: params }).userName({
      where: query,
      select: {
        assignedTo: {
          select: {
